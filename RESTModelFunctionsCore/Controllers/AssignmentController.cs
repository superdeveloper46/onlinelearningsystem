...
namespace RESTModelFunctionsCore.Controllers
{
    ...
    public class AssignmentController : ControllerBase
    {
        ...
        private static VmStudent GetStudentInfoByHash(string hashedPassword)
        {
        ...
        private static VmCodingProblem GetCodingProblem(int codingProblemId)
        {
        ...
        private static List<VmCodingProblemInstance> GetCodingProblemInstance(int codingProblemId, int studentId)
        {
        ...
        private static List<VmVariableValue> GetVmVariableValue(int codingProblemId)
        {
     ...
     VALUES
        ...
        private static ExecutionResult RunCodeForValidation(VmCodingProblem codingProblem, Dictionary<string, string> varPossibleValuePairs)
        {
            if (String.IsNullOrWhiteSpace(codingProblem.Solution))
            {
                throw new Exception("The problem solution is needed in order to verify it.");
            }
            ...
            return HttpRunCodeForValidationRequest(compilerInfo, runInfo);
            //PostRunCodeForValidation
        }
        ...
        private static string GetClassName(string code, VmLanguage codingProblemLanguage)
        {
            ...
            System.Text.RegularExpressions.Regex rgx = new Regex(pattern);
            var match = rgx.Match(code);
            if (!match.Success)
            {
                //throw new ClassOrFunctionNotFoundException("Class name or function not found in: " + code);
                throw new Exception("Class name or function not found in: " + code);
            }
            return match.Groups["token"].Value;
        }
        private static ExecutionResult HttpRunCodeForValidationRequest(CompilerInfo compilerInfo, RunInfo runInfo)
        {
            using (var client = new HttpClient())
            {
                ...
                requestedUrl = _httpContextAccessor.HttpContext.Request.GetEncodedUrl();
                string apiBaseUrl = string.Empty;

                if (requestedUrl.Contains("localhost"))
                {
                    apiBaseUrl = ConfigurationHelper.GetApiBaseURLTest();
                }
                else
                {
                    apiBaseUrl = ConfigurationHelper.GetApiBaseURL();
                }
                client.BaseAddress = new Uri(apiBaseUrl);
                var jsonStringInput = Newtonsoft.Json.JsonConvert.SerializeObject(compilerRunApiInput);
                HttpContent httpContent = new StringContent(jsonStringInput, System.Text.Encoding.UTF8, "application/json");
                var response = client.PostAsync("CompilerValidation", httpContent).Result;
        ...
        private static ExecutionResult HttpRunCodeRequest(CompilerInfo compilerInfo, RunInfo runInfo, HashSet<(string, string, string)> localinstancesData, string studentId)
        {
            using (var client = new HttpClient())
            {
                CompilerRunApiInput compilerRunApiInput = new CompilerRunApiInput();
                compilerRunApiInput.CompilerInfo = compilerInfo;
                compilerRunApiInput.RunInfo = runInfo;
                compilerRunApiInput.instancesData = localinstancesData;
                compilerRunApiInput.StudentId = studentId;

                //client.BaseAddress = new Uri("https://localhost:44396/api/");
                //var uri = new Uri("https://localhost:44396/api/" + "CompilerApi");
                ...
                client.BaseAddress = new Uri(apiBaseUrl);
                var uri = new Uri(apiBaseUrl + "CompilerApi");
                var jsonStringInput = Newtonsoft.Json.JsonConvert.SerializeObject(compilerRunApiInput);
                HttpContent httpContent = new StringContent(jsonStringInput, System.Text.Encoding.UTF8, "application/json");
...
order by ep.PatternOrder asc";
     ...
     VALUES
        ...
        {
            List<LMS.Common.HelperModels.Param> paramss = new List<Param>();
...
where se.SanitizedCodeErrorId = {submissionId}";
        ...
        private static IEnumerable<CompileCodeHint> GetHints(VmSubmission submission)
        {
            List<CompileCodeHint> codeHints = new List<CompileCodeHint>();
            ...
            foreach (VmCodeError codeError in codeErrors)
            {
                ...
                CompileCodeHint codeHint = new CompileCodeHint
                {
        ...
        [HttpPost]
        [Route("RunCode")]
        public IActionResult RunCode([FromBody] Input input)
        {
            CompilerResultInfo result = RunCode(input.Hash, input.CodingProblemId, input.CourseInstanceId, input.Code, input.CodeStructurePoints);
            return Ok(result);
        }
        //POST api/RunCode
        [HttpPost]
        [Route("RunCodeForValidationAPI")]
        public IActionResult RunCodeForValidationAPI([FromBody] RunCodeForValidationInput input)
        {
            try
            {
                ...
                object result = RunCodeForValidation(codingProblem, input.VarValuePairs);
                return Ok(result);
            }
            catch (Exception e)
            {
        ...
        private CompilerResultInfo RunCode(string hash, int codingProblemId, int courseInstanceId, string answer, int codeStructurePonits)
        {
            ...
            CompilerResultInfo ri = new CompilerResultInfo
            {
            ...
            ri.ExeResult = HttpRunCodeRequest(compilerInfo, runInfo, instancesData, studentId);
     ...
     VALUES
        ...
        private static Dictionary<string, String[]> GetCodingProblemVariablesMultiInstance(HashSet<(string varName, string possibleValues, int numberOfInstances)> variables)

        {
            ...
            foreach (var var in variables)
            {
                ...
                if (solutionArray.Any(x => x == null))
                {
                    ...
                    throw new Exception(String.Format("There are less possible values than instances required for the variable with name {0}", var.varName));
                }
     ...
     VALUES
        ...
        //private static Dictionary<string, String[]> GetCodingProblemVariablesMultiInstanceTest(HashSet<(string varName, string possibleValues, int numberOfInstances)> variables)
        //{
        ...
        public static VmCodingProblem GetCurrentCodingProblem()
        {
            return codingProblem;
        }
    ...
    public class TestResult
    {
        ...
        public IEnumerable<string> ActualErrors { get; set; }
        public string Result { get; set; }
        public bool Passed { get; set; }
    }
    public class CompilerResultInfo
    {
        ...
        public ExecutionResult ExeResult { get; set; }
        public GradeInfo GradeTable { get; set; }
        public IEnumerable<CompileCodeHint> CodeHints { get; set; }
        public object KeywordCount { get; set; }
        public string last { get; set; }
    }
    public class GradeInfo
    {
        public int CompilationWeight { get; set; }
        public int CompilationGrade { get; set; }
        public int TestsWeight { get; set; }
        public int TestsGrade { get; set; }
