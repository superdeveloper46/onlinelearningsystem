    ...
    public partial class AddCoursePrerequisite : System.Web.UI.UserControl
    {
        ...
        private void BindGridPrerequisite()
        {
            using (MaterialEntities db = new MaterialEntities())
            {
                ...
                var model = from a in db.CourseCorequisites
                            where a.CourseId == courseId && a.Type == "Prerequisite"
                            select new { a.CorequisiteCourseId, PrerequisiteCourse = a.Course1.Name, a.Active };
        ...
        private void BindGridCorequisite()
        {
            using (MaterialEntities db = new MaterialEntities())
            {
                ...
                var model = (from a in db.CourseCorequisites
                             ...
                             where a.CourseId == courseId && a.Type == "Corequisite"
        ...
        protected void btnCoursePrerequisite_Click(object sender, EventArgs e)
        {
            if (PrerequisiteValidation())
            {
                using (MaterialEntities db = new MaterialEntities())
                {
                    ...
                    {
                        CourseCorequisite coursePrerequisite = new CourseCorequisite()
                        {
                            CourseId = courseId,
                            CorequisiteCourseId = PrerequisiteId,
                            Active = CheckBoxActivePrerequisite.Checked,
                            Type = "Prerequisite"
                        };
                        db.CourseCorequisites.Add(coursePrerequisite);
                        db.SaveChanges();
        ...
        protected void GridViewCoursePrerequisite_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            try
            {
                ...
                using (MaterialEntities db = new MaterialEntities())
                {
                    CourseCorequisite coursePrerequisite = db.CourseCorequisites.Where(x => x.CorequisiteCourseId == id && x.CourseId == courseId).FirstOrDefault();
                    //TODO: Make sure this is correct
                    db.CourseCorequisites.Remove(coursePrerequisite);
                    db.SaveChanges();
