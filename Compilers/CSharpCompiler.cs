...
namespace Compilers
{
    public class CSharpCompiler : CloudCompiler
    {
        ...
        public override ExecutionResult Run(RunInfo runInfo, CompilerInfo compilerInfo)
        {
            ...
            string testToolDirectory = $@"{CurrentFolder.TrimEnd('\\')}\Binaries\{compilerInfo.TestToolDirectory}";
            string sourceFile = $@"{tempFolder}\{runInfo.StudentId}.{compilerInfo.SourceExtension}";

            string compiler = $@"{CurrentFolder.TrimEnd('\\')}\Binaries\{compilerInfo.CompilerDirectory}\{compilerInfo.Compiler}";
            ...
            string arguments = $@"{sourceFile} -target:library /out:{executable} " +
                                    $@"/r:mscorlib.dll " +
                                    $@"/r:{CurrentFolder.TrimEnd('\\')}\Binaries\BuildTools\Common7\IDE\Extensions\TestPlatform\Microsoft.VisualStudio.QualityTools.UnitTestFramework.dll " +
                                    $@"-nologo";
